...
function Broyden(B, w, c, A, b, mu_wk, sig, m, n, eps, sig_max, it_max)
  N = size(B)[2]
  B0 = copy(B)
  w0 = copy(w)
  status = false
...
#  Fw = vcat(zeros(m), zeros(n), diagm(x)*diagm(s)*ones(n))
  ...
  while true
    ...
    println("Alfa = ", alpha)
    println("Sigma = ", sig)
    mu_wk = (1/n)*w_n[1:n]'*w_n[n+m+1:2n+m]
    w_n, b_status = Broyden(J, w + alpha*d,c, A, b, mu_wk, sig, m, n, eps, sig_max, it_max)
#    println("w_n = ")
#    display(w_n)
    println("mu=", (1/n)*w_n[1:n]'*w_n[n+m+1:2n+m])
...
function Encontrar_Ponto_Inicial_alt(c, A, b, M = 1.0, mu = 10.0, eps = 1.0e-4, it_max=1000)
  m, n = size(A)
  F(w) = begin
    x = w[1:n]
    lamb = w[n+1:n+m]
