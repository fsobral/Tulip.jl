...
function compute_step!(mpc::MPC{T, Tv}, params::IPMOptions{T}) where{T, Tv<:AbstractVector{T}}
    ...
    mpc.αp, mpc.αd = max_step_length_pd2(pt.x, Δ.x, z, dz) # Espaço para otimizar!
    if mpc.αp <= mpc.αd
        mpc.αd = mpc.αp
    else
        mpc.αp = mpc.αd
    ...
    # Corrector
    @timeit mpc.timer "Corrector" Quasi_Newton_Corrector!(mpc, z, dz)
    #compute_corrector!(mpc::MPC)
    mpc.αp, mpc.αd = max_step_length_pd(mpc.pt, mpc.Δc)
    # Pedro: aqui tambem foi modificado para adotar um alpha unico para ambos os problemas primal e dual
    if mpc.αp <= mpc.αd
        mpc.αd = mpc.αp
    else
        mpc.αp = mpc.αd
...
function max_step_length_pd(pt::Point{T, Tv}, δ::Point{T, Tv}) where{T, Tv<:AbstractVector{T}}
    ...
    αp = max(min(one(T), axl, axu), 0)
    αd = max(min(one(T), azl, azu), 0)

    return αp, αd
end

function max_step_length_pd2(p_x, d_x, p_z, d_z)
    #    axl = max_step_length(pt.xl, δ.xl)
    #    axu = max_step_length(pt.xu, δ.xu)
    #    azl = max_step_length(pt.zl, δ.zl)
    #    azu = max_step_length(pt.zu, δ.zu)

    αp = max(min(max_step_length(p_x, d_x), 1.0), 0)
    αd = max(min(max_step_length(p_z, d_z), 1.0), 0)
